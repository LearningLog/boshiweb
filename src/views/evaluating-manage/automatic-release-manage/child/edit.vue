      ...
      <div class="content clearfix">
        ...
        <div class="fr count">
          ...
          <div id="btnGroup">
            ...
            >下一步</el-button>
            <el-button
              v-if="activeStep === 2 && exam.auto_status !== 2"
              v-no-more-click
              type="primary"
              @click="save('exam2')"
            >保存</el-button>
...
import {
  ...
  submit,
  saveThenPublish
} from '@/api/evolution-manage/automatic'
...
export default {
  ...
  methods: {
    // 获取详情数据
    getInitData() {
      getDetail({ _id: this.id }).then(res => {
        const { data } = res
        for (var key in this.exam) {
          delete this.exam[key]
        }
        this.exam = data
        // $.extend(true, this.exam, this.exam2)
        this.exam.selectCompanyId = this.exam.groupId
        this.exam.revolution = data.revolution || 0
        this.exam.beginday_cycle = data.beginday_cycle || 4
        this.exam.begintime_cycle = data.begintime_cycle || '00:00:00'
        this.exam.endday_cycle = data.endday_cycle || 4
        this.exam.endtime_cycle = data.endtime_cycle || '23:59:59'
        this.exam.start_time = data.start_time || ''
        this.exam.end_time = data.end_time || ''
        this.exam.topic_unrest = !!data.topic_unrest
        this.exam.label_id = data.label_id
        this.exam.skill_id = data.skill_id
        this.exam.select_type = data.select_type
        this.exam.total_number = data.total_number || [0, 0]
        this.exam.single_select = data.single_select || [0, 0]
        this.exam.multi_select = data.multi_select || [0, 0]
        this.exam.judge_select = data.judge_select || [0, 0]
        this.exam.single_and_easy = data.single_and_easy || [0, 0]
        this.exam.single_and_normal = data.single_and_normal || [0, 0]
        this.exam.single_and_hard = data.single_and_hard || [0, 0]
        this.exam.multi_and_easy = data.multi_and_easy || [0, 0]
        this.exam.multi_and_normal = data.multi_and_normal || [0, 0]
        this.exam.multi_and_hard = data.multi_and_hard || [0, 0]
        this.exam.judge_and_easy = data.judge_and_easy || [0, 0]
        this.exam.judge_and_normal = data.judge_and_normal || [0, 0]
        this.exam.judge_and_hard = data.judge_and_hard || [0, 0]
        ...
        } else if (this.exam.revolution_type === 3) {
    ...
    valid() {
      ...
      this.exam.target_user = this.dataIsChange2
        ? this.target_user
        : this.exam.target_user
      return true
    },
