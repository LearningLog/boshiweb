...
export default {
  ...
  watch: {
    ...
    pathNavData() {
      this.currentFileId = this.pathNavData.length > 0 ? this.pathNavData[this.pathNavData.length - 1].id : this.listQuery.ownerId
    },
    fileUploadPara: {
  ...
  methods: {
    ...
    classifyNodeIds() {
      ...
      this.$router.push({ path: '/knowledge-base/group-base/list', query: { path: '', ownerId: this.listQuery.ownerId }})
      this.enterFloderByQueryPath()
    ...
    goback() {
      ...
      this.$router.push({ path: '/knowledge-base/group-base/list', query: { path: str, ownerId: this.listQuery.ownerId }})
    },
    ...
    // 授权
    authorize_fn(row) {
      this.$router.push({ path: '/user-center/skill-manager/authorize', query: { id: row.inc }})
    },
